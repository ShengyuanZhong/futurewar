...
from app.domain.constants import ActionType, TaskType, SUMMON_TREASURE_CODES
...
class SelfEvolveController:
    ...
    def _budget_blocks_cmd(self) -> bool:
        ...
        if timeout <= 0:
            return False
        if self.state.self_evolve_steps < timeout - 1:
            return False
    ...
    def _archive_skill(self, type_key: str, task_type: str, entry: dict):
        ...
        """
        question = self.state.self_evolve_first_question or entry.get("task", "")
    ...
    def _budget_hint(self) -> str:
        ...
        if timeout <= 0:
            return ""
        used = self.state.self_evolve_steps
