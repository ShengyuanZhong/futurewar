...
def build_self_evolve_prompt(task_desc: str, context, steps_used: int = 0, timeout_rounds: int = 0, sop_hint: str = None) -> str:
    ...
    return (
        ...
        "# 行动准则\n"
        f"当前阶段：{step}。请按顺序推进：\n"
        "1. 找到并阅读任务书（沙盒内某处的 `task_*.md`），明确三件事：任务目标、需要调用的服务或接口、答案的提交格式。\n"
        ...
        "   - 调用 HTTP 接口：`curl -s -H \"<认证头>\" \"http://...\"`。首次调用直接输出**完整原始响应体**，"
