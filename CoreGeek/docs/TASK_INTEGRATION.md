...
- 提示词移植到 `app/service/task_prompt.py`。保留文件定位、API 原始响应、错误修参、CRLF 修复、提交格式自检和最近20条历史等内容。基准补充了 `skill_hint` 注入；0.4.1进一步增加同服务知识复用、JSON层级核对、多行Python引用和动态回合预算，原文件保持不变。
